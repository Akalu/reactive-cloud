...
public class SecurityConfiguration {
	...
	public SecurityWebFilterChain springSecurityFilterChain2(ServerHttpSecurity http) {
		return http.csrf().disable().httpBasic().and().authorizeExchange().anyExchange().authenticated().and().cors()
				.and().build();
	}
	...
	public MapReactiveUserDetailsService userDetailsService() {
		UserDetails user = User.withUsername("user").password("{noop}user").roles(Role.USER.name(), Role.ADMIN.name())
				.build();
